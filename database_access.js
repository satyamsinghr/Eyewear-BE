module.exports = app => {
    ...
    router.post("/signOut", async (req, res) => {
        try {
            await UserLoginSession.destroy({ 'where': { token: req.headers.authorization } });
            return res.status(200).send({
    ...
    router.delete("/collection", verifyToken, async (req, res) => {
        try {
            const collectionDeleted = await Collection.destroy({ 'where': { id: req.body.Coll_id } });
            if (!collectionDeleted) res.status(500).send({ message: "Internal server data" });
            return res.status(200).send({
    ...
    router.get("/box", verifyToken, async (req, res) => {
        try {
            const BoxesData = await Boxes.findAll({ 'where': { UserId: req.query.userId } });
    ...
    router.delete("/box", verifyToken, async (req, res) => {
        try {
            const boxDeleted = await Boxes.destroy({ 'where': { id: req.body.Box_id } });
            if (!boxDeleted) res.status(500).send({ message: "Internal server data" });
            return res.status(200).send({
        ...
        try {
            ...
            const lensData = sheetData.map(row => {
                const lensEntry = {
                    ...
                    RCylinder: row.RCylinder !== undefined ? row.RCylinder.toString() : '',
                    ...
                    LCylinder: row.LCylinder !== undefined ? row.LCylinder.toString() : '',
    ...
    router.get("/lens", verifyToken, async (req, res) => {
        try {
            ...
            const whereCondition = Object.keys(filter).length > 0
                ? {
                    [Op.and]: Object.keys(filter).map((key) => ({ [key]: filter[key] })),
    ...
    router.delete("/lens", verifyToken, async (req, res) => {
        try {
            const lensDeleted = await Lenses.destroy({ 'where': { id: req.body.Lens_id } });
            if (!lensDeleted) res.status(500).send({ message: "Internal server data" });
            return res.status(200).send({
    ...
    router.get("/patient", verifyToken, async (req, res) => {
        try {
            const patientData = await Patient.findAll({ 'where': { UserId: req.query.userId } });
            if (!patientData) res.status(500).send({ message: "Internal server data" });
            return res.status(200).send({
    ...
    router.get("/patientById", verifyToken, async (req, res) => {
        try {
            const patientData = await Patient.findOne({ 'where': { id: req.query.id } });
            if (!patientData) res.status(500).send({ message: "Internal server data" });
            return res.status(200).send({
                message: 'Patient data',
                Patient_Data: patientData
    ...
    router.put("/block", verifyToken, async (req, res) => {
        try {
            let lensUpdated = await Lenses.update({ Patient_id: req.body.patient_id, Is_Blocked: true }, { 'where': { id: req.body.lens_id } });
            if (!lensUpdated) res.status(500).send({ message: "Internal server data" });
